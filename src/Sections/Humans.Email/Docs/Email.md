...
| ShiftSignupId | Guid? | Bare cross-section id (ShiftSignup, Shifts) — no FK constraint, no nav. Nothing writes it and nothing queries it: the per-signup dedup it exists for was specified and never built (see `Docs/health.md` §5) |
