...
- The **Outbox Pause Flag** is a `system_settings` row keyed `IsEmailSendingPaused` (owned by the Settings section) that, when `"true"`, causes `ProcessEmailOutboxJob` to skip all delivery attempts on its next tick. Resuming flips it back to `"false"`.
- **Email Body Composition** happens entirely inside the section: `IEmailBodyComposer` / `BrandedEmailBodyComposer` are internal, as are `IEmailRenderer` / `EmailRenderer` and the two transports. Business code outside the section builds an `EmailMessage` through `IEmailMessageFactory` — being moved section-side one template at a time by peterdrier/Humans#1651 — and hands it to `IEmailService`. Authorized UI previews can pass an always-send system message to the read-only `IEmailPreviewServiceRead`; it returns the exact canonical branded wrapper without enqueueing it. The section-internal `IEmailPreviewService.RenderMarkdown` is the second, generic preview seam behind the shared `_EmailComposer` component (Base), consumed only by this section's own `EmailPreviewController`: it renders arbitrary human-typed subject/Markdown through `SanitizedMarkdownRenderer` and the same branded wrapper, adding a placeholder (`#`) unsubscribe footer when the caller-supplied `MessageCategory` is opt-outable — no outbox row is created either way.
- **Every human-composed send renders through `SanitizedMarkdownRenderer`** (Base) — Markdig → `Ganss.Xss.HtmlSanitizer`, one pipeline, no bespoke HTML-encode/`<br>` path. Images (`<img src>`) are restricted to `https://` — `data:`, `http:`, and `javascript:` sources are stripped via the sanitizer's `FilterUrl` hook; `<a href>` keeps the sanitizer's broader allowance. `style` attributes are stripped outright (`AllowedAttributes.Remove("style")`) so a CSS `url(...)` (e.g. `background-image`) can't smuggle a non-`https://` fetch the `<img>` rule doesn't cover. `EmailInlineStyler` (section-internal) then walks the rendered HTML with AngleSharp and stamps inline `style` attributes matching `BrandedEmailTemplate`'s palette onto h1–h3/p/a/ul/ol/li/blockquote/table/th/td/hr/code/pre, since many clients strip `<style>` blocks; it runs once, inside `BrandedEmailTemplate.Wrap`, so every branded send and preview gets it for free.
...
### Settings key this section reads and writes
...
| `POST /Email/PreviewMarkdown` | Any authenticated human | `EmailPreviewController.PreviewMarkdown` — renders the caller's own typed subject/Markdown through the section-internal `IEmailPreviewService.RenderMarkdown`, for the `_EmailComposer` "Preview" button. Separate controller from `EmailController` because that one is `AdminOnly` at the class level and every compose form (not just admin ones) needs this. |
...
- `IEmailPreviewServiceRead` is the cross-section seam for side-effect-free final-body rendering: `RenderSystemMessage(EmailMessage)` accepts only always-send system messages (opt-outable messages need recipient-specific unsubscribe policy and are rejected) for previewing a fully-built factory message. `IEmailPreviewService` (section-internal, extends it) adds `RenderMarkdown(subject, markdownBody, category)`, which renders arbitrary human-typed Markdown for the shared composer's "Preview" button and accepts any category, substituting a placeholder unsubscribe footer when it is opt-outable — its only consumer is `EmailPreviewController`, in this section. Both delegate to the same internal `IEmailBodyComposer`.
- **Daily send counts (#1195):** `EmailOutboxProcessor` increments `email_daily_send_counts` at `(now.InUtc().Date, TemplateName)` once per send attempt — `SentCount` on success, `FailedCount` on failure — right after the matching `MarkSentAsync`/`MarkFailedAsync` call, so it inherits the same test-address exclusion (the `continue` for `@localhost`/`@ticketstub.local` fires before either). That increment is wrapped in its own try/catch (`EmailOutboxProcessor.TryIncrementDailySendCountAsync`): a tally write failure is logged and swallowed, never allowed to fall into the per-message catch and flip an already-delivered message to `Failed` for retry. The increment runs in its own `DbContext`/`SaveChangesAsync`, separate from `MarkSentAsync`/`MarkFailedAsync` — a crash between the two calls can silently drop one message's contribution to that day's count. Accepted as a documented, narrow-window risk rather than folded into one transaction, consistent with the same swallow-on-failure trade-off. The table is never purged; `CleanupEmailOutboxJob` only ever deletes from `email_outbox_messages`.
- **Daily send counts backfill:** the admin-triggered backfill (`EmailOutboxService.BackfillDailySendCountsAsync`, behind a review step per `memory/process/no-data-backfills.md`, audited via `AuditAction.EmailDailySendCountsBackfilled`) aggregates retained `Sent` outbox rows by `SentAt`'s UTC date into `SentCount`. Per the issue #1195 spec, **failures are never backfilled** — the outbox keeps only a message's final state, not the day (or count) of each attempt, so a failed row's actual failure day can't be reconstructed; `FailedCount` stays `0` for every backfilled row, and a template that only ever failed produces no row at all. It also drops today's UTC date entirely — not just existing (Date, TemplateName) keys for it — so a day the live processor has already started counting is never merged with a historical partial count (which would otherwise permanently undercount that day). Past that, it inserts only (Date, TemplateName) combinations that have no row yet — it never overwrites processor-written or previously-backfilled data, so re-running it is a no-op past the first pass.
...
- **`Humans.Email.Contracts` — everything consumed from outside the section:**
  ...
  - `IEmailPreviewContributor` + `EmailPreviewPersona` + `EmailPreviewSample` — the fan-out a sending
    ...
    contributed samples and the legacy static table while the migration runs; a contributed sample
    supersedes the legacy row with the same id.
  - `TimeSensitiveTemplates` — the four template names that earn an immediate outbox drain and batch
    priority. Public so the section that owns one of those templates names the same constant the
    queue orders on.
...
- **Section-internal abstractions** (`Humans.Email.Services`): `IEmailOutboxService` (admin surface, consumed only by `EmailController`), `IEmailRenderer` / `EmailRenderer`, `IEmailBodyComposer` / `BrandedEmailBodyComposer`, `IEmailTransport` / `SmtpEmailTransport` / `StubEmailTransport`, `EmailInlineStyler` (static, AngleSharp-based inline-style pass called from `BrandedEmailTemplate.Wrap`), `EmailPreviewService` (`IEmailPreviewServiceRead`), `EmailPreviewController` (`[Authorize]`, not `AdminOnly` — hosts `POST /Email/PreviewMarkdown`).
...
- **Resource set:** the `Email_*` keys live in `Humans.Email/EmailResource.{resx,es,ca,de,fr,it}` beside `EmailRenderer`, their one and only renderer. They are *not* this section's page copy — the two admin views carry no localized string at all.
- **Architecture test:** `tests/Humans.Email.Tests/EmailArchitectureTests.cs` pins the `OutboxEmailService` constructor shape, that it takes no Hangfire dependency, and which side of the boundary each connector abstraction sits on. `tests/Humans.Integration.Tests/Controllers/EmailPageRenderTests.cs` is the step-12 render guard.
