...
**Cross-section FKs:** `TeamId` → Teams domain — **FK only**, no navigation property; callers resolve via `ITeamServiceRead`.
...
| VatRate | int | Percentage 0–21 (service-validated range); the line-item UI offers 0 / 10 / 21 |
...
**Cross-section FKs:** `ResponsibleTeamId` → Teams domain — **FK only, no nav property**; the Finance CategoryDetail view reads a stitched-in `ResponsibleTeamName`.
...
**Owning services:** `BudgetService`, `TicketingBudgetService`
**Owned tables:** `budget_years`, `budget_groups`, `budget_categories`, `budget_line_items`, `budget_audit_logs`, `ticketing_projections`
**Status:** (A) Migrated.

- Everything but `Section`, `BudgetResource` and the migrations is `internal` — HUM0034 enforces it. The cross-section surface is the leaf project `Humans.Budget.Contracts`: `IBudgetServiceRead` (five methods, incl. `GetYearByIdAsync` so Expenses can offer the categories of the year a pending report is already booked to), `IBudgetDemoSeeder` (one), the DTOs those name, and the `BudgetYearStatus` / `ExpenditureType` enums. `ITicketingBudgetService` is internal (ruling 43). The leaf is a project rather than a `Contracts/` folder because one consumer is still in Base — `TicketQueryService`.
...
- **Repository shape** — `budget_audit_logs` is append-only (§12); audit rows are written inside each mutation method's own `SaveChanges`, and the repository exposes only the three audit reads.
