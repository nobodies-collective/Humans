...
**Shadow columns (no C# property):** `users.GoogleEmail` is kept on disk as an EF shadow property (`UserConfiguration.cs`) pending a deferred column-drop PR per `memory/architecture/no-drops-until-prod-verified.md`; the canonical read path is `UserInfo.GoogleEmail` (derived from `UserEmail.IsGoogle`). `user_emails` carries no shadow columns; display sorting is alphabetical on `Email`. Analyzer `HUM0001` (`UserEmailLegacyFieldAnalyzer`) rejects any reference to the legacy properties (`User.GoogleEmail`, `UserEmail.IsOAuth`, `UserEmail.DisplayOrder`).
...
- **`.Contracts` leaves:** `AuditLog` (every write path records an audit entry), `Auth` (role assignment on provisioning and merge), `Camps` (camp name + roles on the admin popover), `Campaigns`, `Consent` (profile completeness gates on consent state), `Email` (verification, unsubscribe and merge-notification mail), `Gdpr` (`UserService` and `AccountMergeService` are Article 15 contributors), `Governance` (`IMembershipCalculatorRead` for non-compliant suspension), `GoogleIntegration` (`IGoogleSyncService` drops a suspended human from team resources), `Notifications` (merge requests and email problems notify the human), `Onboarding` (profile editing reports onboarding progress), `Shifts` (participation backfill reads the burn settings), `Teams` (account merge folds team memberships), `Tickets` (participation is derived from ticket sync).
- **Full section projects** — referenced whole because Razor binds a view component's tag helper only from its declaring assembly, so `@addTagHelper` needs the assembly, not its leaf: `Humans.Camps` (`<vc:my-camps />` on `Views/Profile/Index.cshtml`), `Humans.Events` (`<vc:events-card />` on the same view), `Humans.AuditLog` (`IAuditViewerService` in `ProfileController`; `<vc:audit-log>` on `AdminDetail.cshtml`), `Humans.Shifts` (`<vc:shift-signups>` on `Views/Profile/Index.cshtml` and `Views/UsersAdmin/AdminDetail.cshtml`, plus `IStringLocalizer<ShiftsResource>` in `Views/Profile/Edit.cshtml` for its shift-preference strings). Each drops out only when the component moves to the leaf.
