<!-- freshness:triggers
  ...
  src/Humans.Analyzers/UserEmailLegacyFieldAnalyzer.cs
  src/Humans.Analyzers/IdentityFindByEmailAnalyzer.cs
  tests/Humans.Analyzers.Tests/UserEmailLegacyFieldAnalyzerTests.cs
  tests/Humans.Analyzers.Tests/IdentityFindByEmailAnalyzerTests.cs
  tests/Humans.Web.Tests/Services/Gdpr/GdprExportDependencyInjectionTests.cs
...
`src/Sections/Humans.Users` (+ `.Contracts`) covers the **User/Identity**
...
are stated separately, because they are separate invariants.

The agent's `fetch_section_guide` tool reaches this file under the canonical key `Users`, with
`Profiles` and `Profile` as aliases (`AgentSectionKeys`).
...
**Shadow columns (no C# property):** `users.GoogleEmail` is kept on disk as an EF shadow property (`UserConfiguration.cs`); no production code reads or writes it any more (nobodies-collective/Humans#1102 retired the last reader) and the column drop is deferred to a later PR per `memory/architecture/no-drops-until-prod-verified.md`. The canonical read path is `UserInfo.GoogleEmail` (derived from `UserEmail.IsGoogle`). `user_emails` carries no shadow columns; display sorting is alphabetical on `Email`. Analyzer `HUM0001` (`UserEmailLegacyFieldAnalyzer`) rejects any reference to the legacy properties (`User.GoogleEmail`, `UserEmail.IsOAuth`, `UserEmail.DisplayOrder`).

**Profile-adjacent fields** — `GoogleEmailStatus`, `ContactSource`, `ExternalSourceId`, `UnsubscribedFromCampaigns` — are also described under [Part 2 — Profiles](#user-identity-extension) because `CachingUserService` stitches them into `UserInfo`. The canonical field table is here; Part 2 describes `UserInfo` projection semantics.
...
`UserStateEvaluator.Classify`'s `isGdprDeleted` (→ `UserState.Deleted`) is keyed on the minted tombstone email (`deleted-<id>@deleted.local`, written by `ApplyExpiredDeletionAnonymizationAsync`), never on a user-editable name field — `DisplayName`/`BurnerName` are things a member can type, and a burner name of "Deleted User" must not read as erasure (nobodies-collective/Humans#1742). A narrower legacy fallback recognises the complete pre-email tombstone shape — `DisplayName`, `FirstName` and `LastName` all set to their sentinel values together — for rows anonymized before the email scrub existed; it retires with the #1102 `DisplayName` column drop.

`User`'s only navs are `User.UserEmails` (required by the `User.Email` override) and `User.EventParticipations` (owned by this section). Every other relationship is FK-only, configured inverse-side on the owning entity (e.g., `ProfileConfiguration.HasOne<User>().WithOne().HasForeignKey<Profile>(p => p.UserId)`); readers route through `IUserService` / the owning section's service. This is documented, not test-pinned — a test asserting an entity lacks a nav property is forbidden by [`no-tests-for-absences`](../../../../memory/architecture/no-tests-for-absences.md).

`User.Email` is overridden on the entity to compute from `UserEmails` (first verified, primary-preferred) — application code reads `user.Email` and gets the canonical address without touching the underlying Identity column (or `UserInfo.PrimaryEmail` for canonical reads on a singly-loaded User without `UserEmails` hydrated). `User.NormalizedEmail` is `[Obsolete]` (diagnostic id `HUM_USER_NORMALIZEDEMAIL`); applications must use `user.Email` or `IUserEmailRepository` for canonical lookups. `User.EmailConfirmed` is also overridden (true when any `UserEmail` is verified). Identity's email-lookup APIs (`UserManager.FindByEmailAsync` / `FindByNameAsync`) are forbidden by analyzer `HUM0003` — application code routes through `IUserEmailService.FindVerifiedEmailWithUserAsync` / `IMagicLinkService.FindUserByVerifiedEmailAsync` instead. Identity itself does not call these internally, so no custom `UserStore` is needed.

Every newly created User has a corresponding Profile row materialized inline at the User-creation call site (see [Stub Profile invariant](#part-2--profiles)); until the names are filled the user is `UserState.Bare`. Legacy profile-less users are reconciled via `/Profile/Admin/Backfill`.
...
**Table:** `event_participations` (EF configuration: `Data/Configurations/EventParticipationConfiguration.cs`.)
...
The entity carries `TargetUser`, `SourceUser`, and `ResolvedByUser` navigation properties (configured with `HasOne(...).WithMany().HasForeignKey(...)`); the merge admin views read them directly. Strip and route through `IUserServiceRead.GetUserInfosAsync` when this pattern is generalised across the section — see `Docs/health.md`.
...
**`UserController`** (`/User/*`, `[Authorize]`, exempt from `MembershipRequiredFilter` because these are its redirect targets; each action self-checks `UserState`):
- `GET /User/Status` — account-status wall for non-`Active` states
- `GET /User/Deletion` — deletion-pending landing
- `POST /User/Deletion/Cancel` — cancel a pending deletion (`IAccountDeletionService.CancelDeletionAsync`)
...
| Authenticated human | Read own User row. Self-declare `NotAttending` for the active event year (`POST /Profile/Me/DeclareNotAttending` → `IUserService.DeclareNotAttendingAsync`; undo via `/Profile/Me/UndoNotAttending`). Request account deletion. |
...
- `UserEmail.GoogleEmailStatus = Rejected` is terminal for sync-driven writes: `TrySetGoogleEmailStatusFromSyncAsync` targets the user's canonical verified `IsGoogle` address and refuses to flip a `Rejected` address back to `Valid`. The user clears it by selecting a different Google email — a fresh row that starts `Unknown` — so a rejection never strands sync after the address changes. Status is per-address; the `User.GoogleEmailStatus` column is deprecated and has no writer.
...
- `UserService` implements `IUserDataContributor` (design-rules §8) and contributes the User-account slice (Id, Email, DisplayName, PreferredLanguage, GoogleEmail, UnsubscribedFromCampaigns, SuppressScheduleChangeEmails, ContactSource, deletion / created / last-login timestamps) under `GdprExportSections.Account` plus an array of `{ Year, Status, Source, DeclaredAt }` rows under `GdprExportSections.EventParticipations` (every `EventParticipation` row owned by the user) to the GDPR export. It also emits the Profiles-half slices `Profile`, `ContactFields`, `UserEmails`, `VolunteerHistory`, `Languages` and `CommunicationPreferences` — see [Part 2 — Profiles](#part-2--profiles).
- **The cross-section reads resolve merge chains forward, and Users is the only thing that walks one.** `GetUserInfoAsync`, `GetUserInfosAsync` and `GetAllUserInfosAsync` answer a merge-tombstone id with the surviving row (transitively, so A→B→C resolves to C), and drop tombstones from the load-all read entirely, one entry per living human. Every row they return is stamped with `MergedUserIds`, and `UserInfo.AllUserIds` is that row's own id followed by them: the list a chain-following read queries over, seeded from the record returned and never from the id the caller asked with. `SearchUsersAsync` omits tombstones and answers an exact-id query for a merged-away id with the survivor. A cycle or a chain pointing at a row that is no longer there is logged at Warning and resolves to the last row reached, so a bad graph degrades rather than spins.
- **A resolving read hands the caller a different id, and the caller acts on that one.** Whatever a caller does after `GetUserInfoAsync` / `GetUserInfosAsync`, write, look up by id, notify, audit, redirect, it does with `UserInfo.Id` (or `AllUserIds`), not with the id it asked for. A path that must not act for anyone but the id it was handed (the Gate claim, an Asociado ballot stored under an id) compares `info.Id` to the requested id and fails closed. Raw per-id row reads that stamp the owner id (`GetEntitiesByUserIdAsync`, `GetEntitiesByUserIdsAsync`, the email-problems page) stay on the raw reads so rows and the id labelling them come from one row.
- **The raw reads are the escape hatch, and there are exactly two.** `IUserService.GetRawUserInfoAsync` and `GetAllRawUserInfosAsync` return the row as stored, tombstones included, still stamped with `MergedUserIds`. They exist for this section's own merge machinery (`AccountMergeService`, `AccountDeletionService`, the admin screens) and for `RoleAssignmentClaimsTransformation`, which must stamp a tombstone principal `Merged` rather than inherit the survivor's Active state. No other section calls them.
- **The merge index is derived state, rebuilt on demand.** `CachingUserService` builds a `Dictionary<Guid, Guid[]>` over its warmed snapshot the first time a read needs it, and drops it on every mutation of the dict (`TrackedCache.OnMutated`). It is never persisted and never invalidated by hand; a merge, an erasure or a cache refresh all mark it dirty through the same hook. Build and drop share one lock, so a mutation that lands mid-rebuild is not overwritten by the stale index.
...
- `UserInfo.IsActive` excludes tombstones (`IsTombstone`: merged, GDPR-anonymized, or legacy `.local`-email rows) — a tombstone keeps its anonymized Profile row, so "has a profile" alone doesn't exclude it (peterdrier/Humans#1707).
- **A merge is atomic by ordering, not by transaction.** `AccountMergeService.MergeAsync` (reached from `AcceptAsync`) uses no `TransactionScope` — crossing section DB boundaries in one transaction is disallowed. Instead the steps run in a fixed order and each commits independently: move every section's rows via the `IUserMerge` fan-out, settle the pending email, **tombstone the archived account last**. The tombstone is the single observable commit point and the source of truth; a failure before it leaves the archived account untouched and the request still actionable. Every `IUserMerge.ReassignAsync` must therefore stay idempotent — "move all of archived's X to survivor" is a no-op on re-run — so a partial merge is safely retryable. The pending-email settle is deliberately **non-fatal**: a missing or already-consumed pending email is the desired end state, not an error. The only `TransactionScope` in the service is in `RejectAsync`, wrapping the pending-email delete and the request close so a dangling `PendingEmailId` can never block a later accept.
...
- Other Application-layer services **cannot** read or write the `users` / Identity tables directly — they go through `IUserService` (or, inside this section, `IUserServiceInternal`).
...
- **On scheduled deletion expiry:** `ProcessAccountDeletionsJob` calls `IAccountDeletionService.AnonymizeExpiredAccountAsync` per due user. It captures the identity slice the caller needs for the confirmation email, resolves the merge chain, and calls `IGdprService.EraseForUserAsync` per id — the erasure fan-out lives in Gdpr beside the export one, so the loop over `IUserDataContributor`s and the `GdprExportSections.Account`-last ordering are Gdpr's concern. A contributor that throws aborts the run with the deletion markers still set, so the whole cascade retries the next day. Each archived id's `UserInfo` cache entry is dropped as its erasure completes (interleaved, so a mid-chain failure still invalidates the ids already erased); then the cross-cutting caches (Teams active-teams + member, role-assignment claims, shift authorization, shift view) — the survivor's `UserInfo` entry refreshes automatically as part of the `IUserService` write path. The job writes the `AuditAction.AccountAnonymized` audit entry and sends the confirmation email.
- **On admin purge (non-Production only):** `UsersAdminController.PurgeHuman` (`POST /Users/Admin/{id}/Purge`) calls `IAccountDeletionService.PurgeAsync`. The orchestrator delegates the actual identity collapse to `IUserService.PurgeOwnDataAsync` (anonymizes the `users` row, drops `UserEmail` rows and `AspNetUserLogins` rows, refreshes the cached `UserInfo` entry), then drops the Teams active-teams cache and per-user role-assignment / shift-authorization caches. Identity-only — does not cascade to Profile rows.
- **On account merge accept:** `IAccountMergeService.AcceptAsync` (Users section) resolves the request and delegates to the one merge engine, `MergeAsync(survivor, archived, …)` — the admin picks the survivor, so the request's stored direction can be flipped. `MergeAsync` fans out its `IUserMerge` implementations to re-FK source → target (e.g. `IUserService.ReassignLoginsToUserAsync` for `AspNetUserLogins`, `ReassignEventParticipationToUserAsync` for `event_participations`), then `AnonymizeForMergeAsync` to tombstone the source row by setting `MergedToUserId` + `MergedAt` and bumping `LockoutEnd` far-future so the source can no longer sign in. The source `User` row stays in place as a redirect — it is NOT deleted — so append-only history written under the source id (audit log, consent records, budget audit log) remains attributable.
- **On per-user reads of a target after merge:** every `UserInfo` the cross-section reads hand out carries `MergedUserIds`, every id whose `MergedToUserId` chain reaches this row, transitively, sorted. Append-only sections (`IAuditLogService`, `IConsentService`, `IBudgetService.ContributeForUserAsync`, `IExpenseService.ContributeForUserAsync`) query over `AllUserIds` of the record they got back, so source-tombstoned rows surface for the target and a read through a source id sees the target's own rows. Nobody outside Users walks `MergedToUserId`.

## Cross-Section Dependencies

Outbound (Users → other sections). `Humans.Users.csproj` references, all acyclic — none of the targets names a `Humans.Users` type (they reference `Humans.Users.Contracts` at most):

- **`.Contracts` leaves:** `AuditLog` (every write path records an audit entry), `Auth` (role assignment on provisioning and merge), `Camps` (camp name + roles on the admin popover), `Campaigns`, `Consent` (profile completeness gates on the consent state), `Email` (verification, unsubscribe and merge-notification mail), `Gdpr` (`UserService` and `AccountMergeService` are Article 15 contributors), `Governance` (`IMembershipCalculatorRead` for non-compliant suspension), `GoogleIntegration` (`IGoogleSyncService` drops a suspended human from team resources; `ITeamResourceService` verifies a coordinator's team Group before team-reply messaging), `Notifications` (merge requests and email problems notify the human), `Onboarding` (profile editing reports onboarding progress), `Shifts` (participation backfill reads the burn settings), `Teams` (account merge folds team memberships), `Tickets` (participation is derived from ticket sync).
- **Full section projects** — referenced whole because Razor binds a view component's tag helper only from its declaring assembly, so `@addTagHelper` needs the assembly, not its leaf: `Humans.Camps` (`<vc:my-camps />` on `Views/Profile/Index.cshtml`), `Humans.AuditLog` (`IAuditViewerService` in `ProfileController`; `<vc:audit-log>` on `AdminDetail.cshtml`), `Humans.Shifts` (`<vc:shift-signups>` on `Views/Profile/Index.cshtml` and `Views/UsersAdmin/AdminDetail.cshtml`, plus `IStringLocalizer<ShiftsResource>` in `Views/Profile/Edit.cshtml` for its shift-preference strings). Each drops out only when the component moves to the leaf.
...
`UnsubscribeService` → `ICommunicationPreferenceService.UpdatePreferenceAsync` stays inside the section. `CommunicationPreferenceService` writes its own table; `UserInfoSaveChangesInterceptor` pattern-matches `CommunicationPreference` saves and refreshes the affected `UserInfo` entry (`RefreshCommunicationPreferencesAsync`), so no explicit invalidation call is needed.

Inbound (other sections → Users) — the typical direction:

- **Tickets / Gate:** call `IUserService.SetParticipationFromTicketSyncAsync` (`TicketSyncService`, `AttendeeContactImportService`, `GateService`), `RemoveTicketSyncParticipationAsync`, `BackfillParticipationsAsync`. Self-declaration is this section's own `ProfileController` (`/Profile/Me/DeclareNotAttending`). Direct writes to `event_participations` are forbidden.
...
- **Audit Log / Consent / Budget / Expenses:** read `UserInfo.AllUserIds` off the row `GetUserInfoAsync` / `GetUserInfosAsync` already returned, to chain-follow merge tombstones on per-user reads of append-only entities.
...
- `IUserRepository` (impl `Humans.Users/Data/Repositories/UserRepository.cs`) owns the SQL surface for `users` plus `event_participations` (the natural key is User). `IUserEmailRepository` is the parallel surface for `UserEmail` (the Profiles half's table, read/written from the Users half for lookup + OAuth-email lock-step).
- **Caching decorator.** `CachingUserService` (Singleton, `Humans.Users/Data/`) owns a `ConcurrentDictionary<Guid, UserInfo>` of the unified read-model spanning the contributing tables (`users`, `user_emails`, `event_participations`, AspNet `user_logins`, `profiles`, `contact_fields`, `profile_languages`, `volunteer_history_entries`, `communication_preferences`). Dict hits are served synchronously; a cache miss refills via the inner Scoped `UserService` (keyed `"user-inner"`); writes through `IUserService` refresh the affected entry (sign-in `LastLoginAt` flows through `IUserService.RecordLoginAsync`, whose decorator refreshes the entry directly); Identity-machinery writes that bypass the service surface (`UserManager.UpdateAsync`, OAuth `UserEmail` row creation) are caught by `UserInfoSaveChangesInterceptor` (EF `SaveChangesInterceptor`, registered on both the scoped and factory contexts) and routed through `IUserInfoInvalidator.InvalidateAsync`. `CachingUserService` itself implements `IHostedService` (via `TrackedCache`) and populates the dict at startup; load-all reads call `EnsureWarmed` / `EnsureWarmedAsync` and recover transparently if startup warmup hasn't completed. `UserInfo` is the single cache — there is no separate profile cache (`Docs/health.md`).
- **Read/write interface split.** `IUserServiceRead` (`GetUserInfoAsync`, `GetUserInfosAsync`, `GetAllUserInfosAsync`, `SearchUsersAsync`, `GetOnsiteUsersAsync`, `GetAllParticipationsForYearAsync`, `GetAccountsDueForAnonymizationAsync`, `GetByEmailOrAlternateAsync`) is the cross-section read surface — only `UserInfo` / `HumanSearchResult` / `OnsiteUserRow` / `UserParticipationRow` projections plus Guid primitives, no EF entities. `IUserService : IUserServiceRead` adds the writes and reads other sections call (participation from ticket sync, tier changes, merge reassignments, cache invalidation). The section-only half of the write funnel — profile saves, deletion fields, `UserEmail` storage commands, the merge tombstone, the email-problems reads — is the internal `IUserServiceInternal : IUserService`, injected only inside `Humans.Users`; both resolve to the same `CachingUserService`. External sections that only read inject `IUserServiceRead`. Enforcement is advisory pending the Roslyn analyzer; arch tests in `UserArchitectureTests` pin the inheritance + same-singleton DI. See `memory/architecture/section-read-write-split.md`.
- **Identity framework surface** — `AccountController` lives in `Humans.Web/Controllers/` (the Shell), not in this section, and injects `UserManager<User>` / `SignInManager<User>` directly per the §2a exception, as do the Development section's dev-login controller and its account-creating seeders. There is no `AuthController` or `ManageController` class — magic-link orchestration lives in `IMagicLinkService` (Auth section) and account self-management lives across `AccountController` and Profile views. Non-controller code routes through `IUserService`.
- **GDPR:** `UserService` implements `IUserDataContributor` and contributes the `GdprExportSections.Account` and `EventParticipations` slices plus the Profiles-half slices (`Profile`, `ContactFields`, `UserEmails`, `VolunteerHistory`, `Languages`, `CommunicationPreferences`); `ExpectedContributorTypes` in `tests/Humans.Web.Tests/Services/Gdpr/GdprExportDependencyInjectionTests.cs` enforces registration (design-rules §8).
- **Resource set:** `UsersResource.{resx,ca,de,es,fr,it}` at the project root (folder and namespace must agree — nobodies-collective/Humans#1365), covering the whole assembly (both parts of this doc). Views bind it as `Localizer` and `SharedResource` as `SharedLocalizer` in `Views/_ViewImports.cshtml`; `UsersResource.cs`'s remarks record the leave-or-duplicate decision for every key that stayed in `SharedResource`. What stayed and why: `Common_`, `Nav_`, `Dashboard_`, `Validation_` and `Privacy_` are generic; `Admin_` is rendered by the Admin shell frame too; `Application*_`/`ApplicationStatus_` are also rendered by Governance's copy of the tier form; `Teams_`/`MyTeams_`/`TeamDetail_` and `Search_` belong to other renderers; `Enum_EmailOutboxStatus_` is Humans.Email's set, reached through `SharedLocalizer.EnumDisplay`. `Views/Profile/Edit.cshtml` keeps `IStringLocalizer<ShiftsResource>` for its shift-preference strings rather than duplicate the copy. `UserArchitectureTests.SectionTypesLocalizeThroughTheSectionsOwnResourceSet` guards the constructor side; `UsersPageRenderTests` asserts no raw key reaches the HTML, in English and in Spanish.
- **Architecture tests:**
  - `tests/Humans.Users.Tests/Architecture/UserArchitectureTests.cs` — `IUserService_InheritsIUserServiceRead`, `CachingUserService_ImplementsIUserServiceRead`, `IUserService_IUserServiceRead_And_IUserInfoInvalidator_ResolveToSameSingleton` (the decorator is the same singleton under all three keys), and `SectionTypesLocalizeThroughTheSectionsOwnResourceSet`.
  - Analyzer `HUM0001` (`UserEmailLegacyFieldAnalyzer`, tested by `tests/Humans.Analyzers.Tests/UserEmailLegacyFieldAnalyzerTests.cs`) — flags any reference to the legacy shadow properties (`User.GoogleEmail`, `UserEmail.IsOAuth`, `UserEmail.DisplayOrder`).
  - Analyzer `HUM0003` (`IdentityFindByEmailAnalyzer`, tested by `tests/Humans.Analyzers.Tests/IdentityFindByEmailAnalyzerTests.cs`) — flags `UserManager.FindByEmailAsync` / `FindByNameAsync` calls outside the allowed sites; call sites route through `IUserEmailService.FindByAddressAsync` instead.
  - `AccountDeletionService` is covered functionally by `tests/Humans.Users.Tests/Services/AccountDeletionServiceTests.cs`.
...
Per-human personal data: profile, contact fields, emails, communication preferences.
...
- **Duplicate Account Detection** scans for email addresses appearing on multiple accounts (across `User.Email` and `UserEmail.Email`, with gmail/googlemail equivalence). Admins act on candidates from the unified **Account Merges** page (`/Users/Admin/AccountMerges`).
...
`User` has no `GoogleEmail` property; the `users.GoogleEmail` column is an EF shadow property (`UserConfiguration.cs`) kept pending a deferred column-drop PR per `memory/architecture/no-drops-until-prod-verified.md`. The canonical read paths are `UserInfo.GoogleEmail` (derived from the `UserEmail.IsGoogle` row) and `UserInfo.PrimaryEmail`.

Google sync status is per-address on `UserEmail.GoogleEmailStatus` (`GoogleEmailStatus` enum stored as string, default `Unknown`) — it belongs to the address Google rejected, not the user. Set to `Rejected` on a permanent Google API error for that address; selecting a different Google email is a fresh `Unknown` row, so switching address resets sync naturally. The legacy `User.GoogleEmailStatus` column is `[Obsolete]` and retained on disk pending a deferred drop migration; no live reader/writer.
...
`Profile` has no `User` nav. Consumers resolve User data via `IUserServiceRead.GetUserInfosAsync`. Aggregate-local navs `ContactFields`, `VolunteerHistory`, and `Languages` are kept.
...
| `/Profile/Me/Edit` | GET/POST — edit own profile |
| `/Profile/Me/DeclareNotAttending`, `/Profile/Me/UndoNotAttending` | POST — self-declare / undo `NotAttending` for the active event year |
| `/Profile/Me/Emails` | Email management grid |
| `/Profile/Me/Emails/Add` | POST — add an address and send its verification link |
| `/Profile/Me/Emails/Verify` | `[AllowAnonymous]` — consume a verification token |
| `/Profile/Me/Emails/SetPrimary`, `/SetGoogle`, `/SetVisibility`, `/Delete` | POST — per-row flag/visibility edits and delete (`UserEmailOperations.Edit`) |
| `/Profile/Me/Emails/ClearGoogle`, `/Profile/Me/Emails/ClearPrimary` | POST — self-recovery: drop a single row's `IsGoogle`/`IsPrimary` flag (only surfaced in UI on N>1 violation; auth is self-or-admin) |
| `/Profile/Me/Emails/Link/{provider}`, `/Profile/Me/Emails/Unlink/{id}` | POST — start an OAuth link for the signed-in user / unlink a provider-attached row |
| `/Profile/Me/LinkedAccounts/Unlink` | POST — unlink by `(provider, providerKey)`; enforces the auth-method invariant server-side |
| `/Profile/{id}/Admin/Emails` | Per-user email diagnostic (`UserEmailOperations.Edit`) |
| `/Profile/{id}/Admin/Emails/Add`, `/SetPrimary`, `/SetGoogle`, `/SetVisibility`, `/Delete`, `/Unlink/{emailId}` | POST — admin mirrors of the self-service email edits |
| `/Profile/{id}/Admin/Emails/ClearGoogle`, `/Profile/{id}/Admin/Emails/ClearPrimary` | POST — admin remediation: drop a single row's flag without auto-promoting a successor |
| `/Profile/{id}/Admin/Emails/AddVerified` | POST (`PolicyNames.AdminOnly`) — add an address already verified |
| `/Profile/{id}/Admin/Emails/Verify` | POST (`PolicyNames.AdminOnly`) — mark a pending plain UserEmail row verified without consuming a token; creates a merge request when the address is already verified on another account |
| `/Profile/Me/ShiftInfo` | Shift preferences — served by Shifts' `ShiftProfileController`, not this section |
| `/Profile/Me/DietaryMedical` | GET/POST — dietary preference, allergies, intolerances, medical conditions ([spec](features/dietary-medical-nudge.md); dashboard nudge surfaces this when a human has an active 6h+ signup) |
| `/Profile/Me/CommunicationPreferences` | Per-category email/in-app communication preferences |
| `/Profile/Me/CommunicationPreferences/Update` | POST — save one category's preference |
| `/Profile/Me/Notifications` | Permanent redirect to `/Profile/Me/CommunicationPreferences` |
| `/Profile/Me/Privacy` | Privacy / deletion status |
| `/Profile/Me/Privacy/RequestDeletion` | POST — start the 30-day deletion countdown |
...
| `/Profile/{id}/PublicPopover` | `[AllowAnonymous]` — public-page coordinator popover; 404 unless the target coordinates a public-page team ([spec](features/public-coordinator-popover.md)) |
| `/Profile/{id}/SendMessage` | GET/POST — send facilitated message |
| `/Profile/Search` | People search |
| `/Profile/Picture` | `[AllowAnonymous]` — profile picture bytes |
...
| `/Profile/Admin/Backfill` (GET), `/Profile/Admin/Backfill/Run` (POST) | Stub-Profile backfill tool — idempotent count-and-bulk-create for profile-less users (`ProfileBackfillAdminController`, `AdminOnly`) |
...
- App access is gated by the stored `User.State` (`UserState`: Bare/Active/DeletePending/Suspended/Rejected/Deleted/Merged/AdminSuspended). `Profile` carries no state column.
...
- Data export returns all personal data as a JSON download (GDPR Article 15). `AccountMergeService` is an `IUserDataContributor` per design-rules §8a (the `AccountMergeRequests` slice); the profile slices are emitted by `UserService`. `ProfileService` implements no contributor interface. The orchestration lives in `GdprService`.
...
- `AccountMergeService` writes and `DuplicateAccountService` reads go through this section's repositories and `IUserService` — never through cross-section `DbSet` reads.
...
- Facilitated messaging (`/Profile/{id}/SendMessage`) is a **conduit, not a mailbox** — Humans relays one-off plain-text emails between humans and stores nothing: no message persistence, no threading, no attachments. (Deliberate contrast with Feedback, which *does* persist a conversation thread.) The body is plain text only — HTML tags are stripped server-side, then the renderer HTML-encodes the text and converts newlines to `<br>`; max 2000 chars. The recipient's address is never disclosed to the sender. A coordinator may send with `Reply-To` set to a team Google Group only when `TeamMessageOptionsProvider` (`Services/`, an orchestrator over `ITeamServiceRead` and `ITeamResourceService`, no tables) offers the team: the viewer coordinates an active team with a Google Group prefix, and that team has an active `Group` resource with a successful sync, no error, and a `Url` equal to the group's `https://groups.google.com/a/{domain}/g/{prefix}` (the one field both GoogleIntegration link paths write consistently). `ProfileViewController` only looks the requested team up in that list, on GET and on POST alike, and sorts it for display. Personal sends set `Reply-To` to the sender's notification email only when the sender ticks "include my contact info". Every send uses `AuditAction.FacilitatedMessageSent`; team sends name and link the team in the audit entry without copying its address into the human's audit description.
- The shared own/other profile view renders `<vc:user-parts user-id>` (`UserPartsViewComponent`), which collects every active section's `IUserPart` descriptors for the target user, orders them by weight, and invokes each named component with the profile target as `userId`; a contributor that throws is logged and skipped. Contributing components own their data and any visibility beyond the ordinary authenticated-profile gate; an empty component renders nothing. Users references no contributor: Events is the reference contributor and reaches the page only through this seam.
...
- **`UserInfo` is canonical.** Derived properties `PrimaryEmail`, `AllVerifiedEmails`, `GoogleEmail` are the email read sites; `CachingUserService` populates them from already-loaded `UserEmail` rows (no new repo lookups). The lifecycle marker is the stored `User.State`, surfaced as the non-nullable `UserInfo.State`; `ProfileInfo` carries no state at all.
- **Stub Profile invariant.** Every newly created User materializes a bare Profile inline at the User-creation call site (`ExternalLoginService.CompleteExternalLoginAsync`, `AccountProvisioningService.FindOrCreateUserByEmailAsync`/`CompleteMagicLinkSignupAsync`); the User row carries `UserState.Bare`. `IProfileEditorService.SaveProfileAsync` (via `IUserService.SaveProfileAsync`) promotes `User.State` to `Active` once `BurnerName`/`FirstName`/`LastName` are all populated. Legacy profile-less users (contact imports) are reconciled through the `/Profile/Admin/Backfill` admin tool — idempotent count-and-bulk-create page; no-op when N=0. Until the backfill is run, `GET /Profile/{id}/Popover` renders a sparse fallback card for these users so `<human-link>` hovers don't 404 — see the Invariants section bullet.
- **Names live on `User`, dual-written from `Profile` (nobodies-collective/Humans#1097).** `UserRepository`'s profile `AddAsync`/`UpdateAsync` seam copies `BurnerName`/`FirstName`/`LastName` onto the owning `users` row in the same save; the merge, purge and GDPR-anonymize paths overwrite them with their tombstone labels so no purged name can resolve. `Profile` remains the read source for the legal name; only `UserInfo.BurnerName` reads the `users` column first. Legacy rows are reconciled through `/Profile/Admin/NameBackfill` — idempotent review → confirm page reporting the unsynced count; retiring the `Profile` columns (nobodies-collective/Humans#1098) waits on that page reading 0 in production.
- **`UserEmail.IsPrimary` invariants.** Service-layer guarantee in `UserEmailService.EnsurePrimaryInvariantAsync` (exactly one verified `IsPrimary` per user, recovers from zero/multi states). Account-merge fold preserves target's `IsPrimary` and demotes the source's. No DB unique index — per `memory/architecture/db-enforcement-minimal.md` the service is the contract; a DB partial unique index would push violations to runtime as untyped `DbUpdateException` failures rather than service-layer recovery (column persists under legacy name `IsNotificationTarget` per `no-column-drops-for-decoupling.md`). **`UserEmailService.ClearPrimaryAsync` is the duplicate-flag recovery path** — it drops `IsPrimary` from a single row *without* invoking `EnsurePrimaryInvariantAsync`, but only when at least one other verified `IsPrimary` row exists; it returns `false` otherwise so the user can never end in a zero-verified-primary state via this path. Surface (admin and self): `/Profile/{id}/Admin/Emails/ClearPrimary` and `/Profile/Me/Emails/ClearPrimary` — both UI buttons appear only when ≥ 2 verified `IsPrimary` rows exist, and the service rejects direct form replay below that threshold.
...
- **No cross-domain navs:** `Profile`, `UserEmail` and `CommunicationPreference` carry no `User` nav. Display stitching routes through `IUserServiceRead.GetUserInfosAsync`.
- **GDPR:** `AccountMergeService` implements `IUserDataContributor` (design-rules §8a) and emits the `AccountMergeRequests` slice. The `Profile`, `ContactFields`, `UserEmails`, `VolunteerHistory`, `Languages` and `CommunicationPreferences` slices are emitted by `UserService` — `ProfileService` implements no contributor interface. Section keys are constants on `GdprExportSections`. The `ExpectedContributorTypes` in `tests/Humans.Web.Tests/Services/Gdpr/GdprExportDependencyInjectionTests.cs` enforces registration.
- **Architecture tests** — `tests/Humans.Web.Tests/Services/Gdpr/GdprExportDependencyInjectionTests.cs` (the DI wiring; the orchestrator's own tests are in `tests/Humans.Gdpr.Tests/`).
