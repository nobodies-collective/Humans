...
- **Governance (tier applications + roles):** `TierApplicationApproved`, `TierApplicationRejected`, `TierDowngraded`, `TierTermExpiryCorrected`, `RoleAssigned`, `RoleEnded`.
...
- **City Planning:** `CityPlanningPlacementOpened`, `CityPlanningPlacementClosed`, `CityPlanningContainerPlacementOpened`, `CityPlanningContainerPlacementClosed`, `CityPlanningLimitZoneUpdated`, `CityPlanningLimitZoneDeleted`, `CityPlanningOfficialZonesUpdated`, `CityPlanningOfficialZonesDeleted` — the settings writes that decide who may move barrios and containers on the site map and what the published ground truth is, written by `CityPlanningService`. `EntityType` is `CityPlanningSettings` and `EntityId` the year's settings row; the row records *when* a value changed, so these are the only record of *who*. A rejected zone upload never reaches the row and writes no entry.
- **Expenses / IBAN:** `ExpenseCreatedOnBehalf`, `ExpenseEditedOnBehalf`, `ExpensePayeeIbanUpdated`, `ExpenseSubmit`, `ExpenseEndorse`, `ExpenseCoordinatorReject`, `ExpenseApprove`, `ExpenseReject`, `ExpenseWithdraw`, `ExpenseCategoryOverride`, `ExpenseSepaSent`, `ExpenseSepaReopened`, `ExpensePaid`, `ExpenseAttachmentUploaded`, `ExpenseAttachmentRemoved`, `IbanSet`, `IbanRemove`, `IbanReveal`, `ExpenseHoldedPushed`, `ExpenseHoldedFailed`, `ExpenseHoldedRequeued`. `ExpenseSepaReopened` fires when a FinanceAdmin/Admin reopens a SepaSent report back to Approved after a failed SEPA file download; `IbanReveal` logs plaintext IBAN reveals (expense detail + users admin); `ExpenseHoldedPushed`/`ExpenseHoldedFailed`/`ExpenseHoldedRequeued` track the Holded push outbox (job pushes/write-offs, admin re-queues), written by `ExpenseReportService`. `ExpenseCreatedOnBehalf` and `ExpenseEditedOnBehalf` fire only when a finance admin files or changes a report for another member — create, header edit, and line add/update/remove respectively; self-created and self-edited reports stay unaudited. `ExpensePayeeIbanUpdated` fires when the report's own IBAN page rewrites a pre-approval `PayeeIban` snapshot; its entity is the report, so it lands in that report's on-page history. All of those, plus `ExpenseAttachmentUploaded`/`ExpenseAttachmentRemoved` and the `IbanSet`/`IbanRemove` pair, set `RelatedEntityId` to the member with `RelatedEntityType` `"User"`, which is what carries them into that member's GDPR export — the entity is the report or their Profile and the actor may be the admin, so the related id is the only link back to the subject. An `IbanSet`/`IbanRemove`/`ExpensePayeeIbanUpdated` written by somebody other than its subject names the member and carries the IBAN **unmasked** — the one exception to the masking rule, per `memory/code/audit-pii-subject-allowed.md`.
...
- **Finance / SEPA booking:** `SepaPayoutTransferBooked` — one entry per transfer booked into Holded from `/Finance/Sepa`, written by Finance's `Service.BookSepaTransferAsync`. Same `EntityType` and `EntityId` as the generation entry above; the description adds the Holded payment ids the amount was allocated across, and the IBAN stays **masked**.
...
- **Surveys:** `SurveyCreated`, `SurveyUpdated`, `SurveyOpened`, `SurveyClosed`, `SurveyInvitesSent`, `SurveyReminderSent`, `SurveySubmittedForApproval`, `SurveyApproved`, `SurveyRejected` — survey lifecycle events written by the Survey section.
...
- **Workgroups:** `WorkgroupRegistered`, `WorkgroupReferred`, `WorkgroupRefused`, `WorkgroupWithdrawn`, `WorkgroupClosed`, `WorkgroupReactivated`, `WorkgroupCoordinatorsChanged`, `WorkgroupDispositionRecorded`, `WorkgroupRegisteredExisting`, `WorkgroupApplicationOverdue`, `WorkgroupUpdateDueNotified`, `WorkgroupLogEntryDeleted`, `WorkgroupLogEntryUpdated`, `WorkgroupMeetingUpdated`, `WorkgroupMeetingDeleted`, `WorkgroupCommentHidden`, `WorkgroupsRootFolderUpdated` — the register's lifecycle, written by `WorkgroupService`. Registration is administrative recognition (Board Resolution clause 4), so every step is a human action the Board must be able to see; the edit and delete actions matter because each overwrites a row that keeps no history of its own. `WorkgroupApplicationOverdue` and `WorkgroupUpdateDueNotified` are the daily pass's two notices, written under the job actor. `WorkgroupDormancyFlagged`, `WorkgroupCloseCandidateFlagged` and `WorkgroupDormancyCleared` are reserved — Peter struck the silence machinery on 2026-09-16 and nothing writes them now, but rows an earlier run wrote still read back through them; do not remove.
- **Governance (assembly votes):** `AssemblyVoteOpened`, `AssemblyVoteStopped`, `AssemblyVoteExtended`, `AssemblyVoteCancelled`, `AssemblyVoteClosed`, `AssemblyVotePeeked`, `AssemblyBallotCast`, `AssemblyBallotChanged`, `AssemblyBallotsViewed`, `AssemblyVoteRosterMerged`, `AssemblyVoteRemindersSent` — lifecycle of a binding Asociado vote plus its ballot events (nobodies-collective/Humans#86). The lapse close is written under a job actor; ballot entries name the voter and never the choice; `AssemblyVotePeeked` records an Admin reading the tally before close and is published on the results page; `AssemblyVoteRosterMerged` is written when an account merge finds the merged-away account on a vote's roster and leaves the row and any ballot standing — the record of who was entitled and what they cast is not account state, so the merge reports it rather than folding it; `AssemblyVoteRemindersSent` records the T-24h reminder batch the sweep actually delivered, under the job actor.
<!-- /freshness:auto -->
...
Note: the `/Admin` dashboard's "Recent activity" card is this section's own `AdminActivityCard`, contributed into the Shell's `admin-dashboard` chrome slot (`SectionChrome`) — it renders `AuditLogViewComponent` with `layout="activity", limit=8`. That is a chrome contribution, not route ownership. AuditLog owns no Admin routes.
...
- Per-user reads chain-follow merge tombstones via `UserInfo.AllUserIds` of the record `IUserServiceRead.GetUserInfoAsync(userId)` returned (the survivor's own id plus `MergedUserIds`; never the requested id) so audit entries written under a now-merged source id surface for the fold target, and a read through a merged-away id shows the survivor's rows too. Applies to `GetByUserAsync`, `GetUserAuditLogPageAsync`, the per-entity history filter when entity is a User, and `ContributeForUserAsync` (GDPR). Audit entries are append-only (§12) and stay attributed to the source User row by design, `AnonymizeForMergeAsync` does NOT rewrite `ActorUserId` / `EntityId` / `RelatedEntityId` columns.
...
- **Users/Identity:** `IUserServiceRead.GetUserInfoAsync`, the returned `UserInfo.AllUserIds` chain-follows merge tombstones on every per-user audit read so source-attributed entries surface for the fold target. The read also resolves a tombstone id forward to its survivor, so a per-user audit read keyed on a merged-away id returns the survivor's history.
