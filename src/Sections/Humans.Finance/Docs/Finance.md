...
- **Booking** a transfer settles it against the outgoing Sabadell line that paid it (nobodies-collective/Humans#1185): the bound member's open purchase documents are paid oldest first from the configured treasury account, dated the bank line, and whatever they do not cover is one journal entry, debit the creditor account, credit the bank's ledger account. A recurring sweep is the normal trigger — it matches unbooked transfers to bank lines and books them unattended every two hours; the Book button on `/Finance/Sepa` does the same thing on demand, once a matching line has appeared. It stamps `BookedAt`, the acting admin (null = the sweep), `HoldedBankMovementId` and — once Holded confirms the pairing — `ReconciledAt` onto the transfer row. It is the only thing that moves a transfer out of `Generated`, and it cannot be done twice.
...
- **Unbind holds against a push already in flight, in the steady state only.** `ProcessHoldedCreateAsync` spans several Holded calls, so the drain can be mid-push for tens of seconds while an admin clicks Unbind. `EnsureCreditorContactAsync` therefore writes nothing when the member already holds a contact and the binding already carries its 400000xx: the contact id and `Source`/number come off the binding just read, so the only column that would change is `UpdatedAt`, which nothing reads — and writing it would resurrect a binding the admin cleared, from the copy read before they clicked. Not yet safe in general: a binding still missing its number, and `SetCreditorAccountNumAsync`, write real content and can still lose a concurrent delete (nobodies-collective/Humans#995 — the fix is an update-only repository write, not a version column; see [`no-concurrency-tokens`](../../../../memory/architecture/no-concurrency-tokens.md)).
...
- **A linked contact is trusted and never updated in Holded.** Once a binding (or the lazy-seed from a prior report) carries a contact id, `EnsureCreditorContactAsync` uses it as is and makes no Holded call; only a member with no contact gets a POST. Holded's v2 contact PUT is a full replacement — every omitted field resets, `supplier_record` included — and the partial update the push used to send minted a second creditor account next to the member's first (2026-09-21). A legal-name or IBAN change after the first push therefore does not reach Holded until the link-check sync exists (peterdrier/Humans#1777); [`holded-contact-put-is-full-replacement`](../../../../memory/code/holded-contact-put-is-full-replacement.md).
...
- `/Finance/Creditors` defaults to an **owed-only** client-side filter (checked by default) that hides every row with no positive balance; unchecking it reveals settled and negative-balance accounts too. Display-only — it does not change what `GenerateSepaPayoutAsync` will accept.
...
- **The unmasked IBAN is stored only as sent**: the generated XML and `sepa_payout_transfers.Iban`. Every log line, audit entry and cross-section row goes through `IbanFormatter.Mask` ([`iban-mask-in-logs`](../../../../memory/code/iban-mask-in-logs.md)) — including `HoldedCreditorAccountRow.IbanMasked`, which is masked precisely because it crosses a section boundary. A screen shows it in full only to an admin (`/Finance/CreditorStatement`) or to the member whose IBAN it is.
...
- **Booking allocates oldest first and never over-pays a document.** The transfer amount is spread across the bound member's *approved, still-owing* Holded purchase documents in document-date order, `min(remaining, payments_pending)` each, stopping when the amount is allocated. A draft document is never paid — it books nothing to the ledger, so a payment against it would post to a document that does not exist for accounting.
- **Booking settles the balance, not just the documents.** When the open documents total less than the transfer — or there are none, as with a loan the member made — the remainder is one journal entry, debit the transfer's creditor account, credit `Sepa:TreasuryLedgerAccount`, same date and description as the payments. Nothing is refused for lacking a document behind it: the file paid the balance, and the balance is whatever built it.
- **`Sepa:TreasuryAccountId` and `Sepa:TreasuryLedgerAccount` are required to book and never inferred.** Omitting `treasury_id` lets Holded pay from whichever account it defaults to, which is not necessarily the account the SEPA file drew on; the ledger account is the same bank's `572xxxxx`, the credit side of the remainder entry. Either unset, `/Finance/Sepa` says so once for the whole screen and offers no buttons.
- **A booking that fails mid-allocation is retryable, never terminal** (nobodies-collective/Humans#1185). Whether Holded refused a document payment or the journal entry, the ids already accepted are logged and audited (`PARTIAL`), but **nothing is written to the transfer row** — it stays unbooked. The next attempt re-reads what is already posted under the transfer's own tag and continues from there, so nothing is ever posted twice.
...
- **And it must still name the same contact.** The account number alone is not enough: Holded lets two contacts share one 400000xx, so a rebind to a sibling contact slips past the account check. Booking compares the transfer's own `HoldedContactId` — captured at generation — against the binding's, and refuses when they differ; the file paid the contact named on the transfer, and anything else pays the wrong recipient's documents. `/Finance/Sepa` shows the same reason in place of the Book button, so a doomed booking is never offered. A transfer generated before nobodies-collective/Humans#1146 shipped carries no contact id and keeps the account-only behaviour — the guard never retires a row it has no evidence about.
- One `AuditAction.SepaPayoutTransferBooked` entry per booking, written after the save, carrying the amount, the **masked** IBAN, the creditor account, the bank movement id and every Holded id posted. **A partial booking gets one too**, labelled `PARTIAL` — postings an admin (or the sweep) caused are never invisible to the Board.
- **An accepted posting is never thrown away over an unreadable response.** A 2xx means it posted, so `PayPurchaseDocumentAsync` returns `"unconfirmed:{documentId}"` (and `PostLedgerEntryAsync` `"unconfirmed:entry"`) instead of throwing when Holded gives back no readable id. The allocation continues and the transfer books normally; the sentinel lands only in the audit entry now, naming the document a human must eyeball in Holded. Throwing would have lost the payment from the record entirely and left the transfer retryable.
- **The bank line drives booking, not a click.** `BookSepaTransferAsync` takes the bank movement id and re-validates the pairing server-side — amount, account, not already reconciled, not already booked to another transfer, and exactly one unbooked transfer matches — never trusting what was posted. No unique DB index enforces "one line, one transfer": the guard is the in-memory check plus a process-wide gate — every booking on this server runs one at a time, so the second caller reads the row *after* the first stamped it rather than both reading "not booked" and both posting the whole amount ([`audit-log-as-concurrency-safety-net`](../../../../memory/architecture/audit-log-as-concurrency-safety-net.md)). The sweep's `[DisableConcurrentExecution]` only keeps two sweeps apart; a Book click is not in that lock.
- **A run that cannot close the gap does not book.** `owedNow` caps what a resume may post, so a transfer whose account no longer owes the outstanding part posts only part of it. That is audited (`SHORT SEPA booking`) and the row stays unbooked and retryable — Booked never means less than the transfer's amount is in the ledger. The same holds when the row save itself fails after Holded accepted the postings: `PARTIAL` audit entry, row unbooked.
- **Persist before reconcile.** The booking is saved to the row *before* the bank line is reconciled in Holded — the local save is the cheap write, and losing it after Holded already took the money was nobodies-collective/Humans#1185 itself. A reconcile that fails or is refused leaves `ReconciledAt` null and the booking still stands; the sweep retries it.
