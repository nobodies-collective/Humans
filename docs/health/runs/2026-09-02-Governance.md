...
before any scan) finds two unrelated things under one roof: the `applications` aggregate
...
**The section had two live bugs, both member-facing, and neither found by a scan.**
...
Everything else was surface and truth. Three dead surfaces went (a public cross-section
contract method dead end to end, an interface nothing injected, a calculator shortcut with
no caller), eighteen comments said something false, and the section's own docs carried a
controller, an architecture test file and four service interfaces that do not exist. Two
of the section's six docs had no freshness triggers at all.
...
|---|---|---|---|
| 1 | **Term expiry mis-displayed on both member-facing pages.** `Views/Governance/Index.cshtml` and `Views/Governance/Applications/Index.cshtml` each computed `resolvedYear % 2 == 0 ? +1 : +2` from `ResolvedAt`; `TermExpiryCalculator` computes `today.Year + 2`, bumped to the next odd year. A 2026 approval displayed 2027 against a stored 2029. `UserApplicationSnapshot` already carried `TermExpiresAt`, so the fix carries the stored value through and deletes both copies of the arithmetic — which also removed a hardcoded English "31 December". Two tests added at the service seam. | high | **worked** |
...
| 5 | **`MembershipCalculator.HasAnyExpiredConsentsAsync` had no production caller** — a Volunteers-team shortcut over the live `…ForTeamAsync`. Three tests pinned only it. Reviewer-approved; the reviewer also corrected the proposal (the member was on the class, not the interface). | med | **worked** |
| 6 | **Dead `"ConcurrencyConflict"` switch arm** in `GovernanceBoardVotingController.Finalize`, plus six resx entries. The service never returns that key and `Application` carries no concurrency token — [`no-concurrency-tokens`](../../../memory/architecture/no-concurrency-tokens.md) says it never will. | med | **worked** |
| 7 | **Hardcoded English `[Display(Name = …)]` on the member-facing Create form.** `Motivation` and `AdditionalInfo` rendered English labels in every culture through empty `<label asp-for>` tags; new `GovernanceCreate_*Label` keys in all six cultures now carry them, matching the Asociado questions beside them. The third, on `ConfirmAccuracy`, never rendered at all — the view supplies that label — so it was deleted outright. | med | **worked** |
| 8 | **Eighteen false comments.** Unresolvable crefs (`Domain.Enums.ApplicationStatus`, `IMembershipCalculator` from the Contracts leaf, `UsersDbContext`); wrong migrations path; wrong table name (`application_state_histories`); a caching decorator, a Board daily digest, an `IProfileService` and an `Application/Details.cshtml` that do not exist; `GetBoardVotingDashboardAsync` attributed to the wrong interface; `BoardVoteRow` claimed to cross into `OnboardingService`; "nine prefixes" for ten; `IRecurringJob` attributed to Hangfire; and two stacked `<summary>` blocks that left `ReassignApplicationsToUserAsync` undocumented while its prose documented the member below it. No project sets `GenerateDocumentationFile`, so every broken cref here was silent. | med | **worked** |
| 9 | **Governance.md drift** — a `BoardController` row and prose (`authorization.md` already said no such class exists); an architecture test file at a path that does not exist; `IProfileService` / `IProfileService.GetTierCountsAsync` / `ITeamService` / `IUserService` / `ILegalDocumentSyncService` / `IConsentService` named where the code injects the `…Read` variants; a grandfathering bullet naming two retired jobs and claiming four consumers read governance tables directly (`reforge ownership-violations` reports zero); "one pending per tier" where the code enforces one per person, any tier. | med | **worked** |
| 10 | **Feature-doc and guide drift** — a board-voting tier filter documented in three places and never built; "stays as Volunteer" on rejection, which changes no tier; "reverts to Volunteer" on lapse, which downgrades to another held tier first; `/Governance/Applications` given as the submit route (it is `/Create`); a "nightly" system-team sync that runs hourly; the wrong policy on `/Users/Admin/Roles`. | med | **worked** |
| 11 | **Freshness triggers.** `authorization.md` and `data-access.md` had no trigger block at all, so nothing could ever flag them; no doc in the section was triggered by the Contracts leaf; and three docs listed `Services/TermExpiryCalculator.cs`, already inside the section glob. | med | **worked** |
| 12 | **~150 lines of comment that restate the code or narrate history.** Which lane a file moved in, which PR added a line, what "was already arriving transitively"; and roughly sixty xmldoc summaries and `<param>` lines that say the member's name back ("Unique identifier for the application", "Approves this application"). Where a provenance clause wrapped a live constraint, the constraint stayed. | low | **worked** |
| 13 | **`humans.asociados` counts approved applications of both tiers** and publishes them as "Approved asociado members" — `GovernanceMetricsService` reads `applicationStats.Approved`, which excludes only Withdrawn. Three defensible definitions (people currently holding an active Asociado term; approved Asociado applications ever; the profile tier count the sidebar already shows) and no way to pick from the code. | med | **noted → Needs Peter** |
| 14 | **`Application.ReviewStartedAt` is never set.** Private setter, no writer, yet two DTOs and two views carry it to the page as a permanent blank. It belongs to the unbuilt request-more-info flow (target §5), so it is reported, not removed. | low | **noted → Needs Peter** |
...
| 17 | **GDPR erasure is entirely unpinned.** `ScrubFreeTextForUserAsync` is the section's Art. 17 obligation — which free text is cleared, which skeleton is kept, and that Board notes the person wrote about others are cleared too — and no test asserts any of it. The highest-value remaining test gap. | high | **sweep queue** |
| 18 | **Audit entries on approve/reject unpinned**, and the vote-overwrite rule (one row per Board member, updated in place) is pinned only at the repository, not through `CastBoardVoteAsync`. | med | **sweep queue** |
| 19 | **Assert-the-mock tests.** Two `GetRequiredTeamIdsForUserAsync` Colaborador cases and the `HasActiveRolesAsync` pair assert the substitute's own return value. Four filtering tests plus the user-scoping test duplicate repository coverage at the service. | low | **sweep queue** |
| 20 | **Controller authorization attributes unpinned.** The policies themselves are tested; that these controllers and actions carry them is not. | med | **sweep queue** |
| 21 | **Four unprefixed resx keys** in `GovernanceResource.resx`. Counted, not backfilled — a prefix rename is a rename, not a doctor strike. | low | **no change** |

## Worked

Findings 1–12, seven commits:
...
`dotnet test Humans.slnx -v quiet` green across all 47 test projects before the deletion
...
**What did the run get wrong?** Two things. It proposed deleting
...
**What cost the most time?** Solution-wide builds. Seven of them at ~3 minutes each is
21 minutes of a 2.5h budget, and the section's own test project runs in 2 seconds. The
scoped-inner-loop rule exists for this and the run under-used it: only the deletion strike
genuinely needed the full-solution gate.

**What should the next run on this section look at?** The five queued test gaps, GDPR
...
- [ ] **`humans.asociados` counts the wrong thing** (finding 13). `GovernanceMetricsService`
  ...
  years — under the description "Approved asociado members". Three defensible definitions:
...
- [ ] **`Application.ReviewStartedAt` and `RequestMoreInfo`** (findings 14, 15) are two
  ...
  rides through two DTOs to two views as a permanent blank. Build the flow, or delete both?
...
- [ ] Amendment from the retro: this run spent 21 minutes on solution-wide builds it did not
  need. If the skill's scoped-inner-loop guidance is meant to apply to doctor strikes as
  well as to ordinary work, saying so in Phase 4 would buy back most of that.

## Sweep queue

- `debt: Humans.Governance — GDPR erasure is untested. ScrubFreeTextForUserAsync is the section's Art. 17 obligation (which free text is cleared, which tier/status/date skeleton is kept, and that notes the person wrote as a Board member on others' applications are cleared too) and no test asserts any part of it (finding 17, /section-doctor on Governance 2026-09-02).`
- `debt: Humans.Governance — the audit entries ApproveAsync/RejectAsync write are unpinned, and the one-vote-per-Board-member overwrite rule is pinned only at ApplicationRepository, not through CastBoardVoteAsync (finding 18, /section-doctor on Governance 2026-09-02).`
- `debt: Humans.Governance — assert-the-mock tests: two GetRequiredTeamIdsForUserAsync Colaborador cases and the HasActiveRolesAsync pair assert the substitute's own configured return; four filtering tests plus the user-scoping test duplicate repository coverage at the service layer (finding 19, /section-doctor on Governance 2026-09-02).`
- `debt: Humans.Governance — no test pins that GovernanceApplicationsController.Admin/AdminDetail and GovernanceBoardVotingController carry their authorization attributes. The policies are tested; their placement is not (finding 20, /section-doctor on Governance 2026-09-02).`
...
| Tests | 5, 17–20 |
| History | 12 — 12 false narrations and 29 cuts with trim-to text |
| Comments | 8 and 12 — 18 falsehoods, 84 cuts, and a keeps list the cuts respected |
| Conformance | none — layout PASS, `audit-auth` PASS, `ownership-violations` 0 |
| Inbox | none — `peterdrier/Humans` has three open issues (#1576, #1562, #1554), none Governance; no Governance rows in `debt-ledger.yml`; the section had no `Docs/debt.yml` |

Independence check: **pass**. Findings 1, 4, 14 and 15 came from the target shape rather
than from a scan; findings 1 and 2, the two highest-value items, came from reading views
and service code by hand, and `reforge audit-surface` reported neither.

## Size

reforge score 626 → unchanged in kind; loc 3888 at the start of the run. The run deleted
roughly 400 lines net (three dead surfaces, ~150 lines of comment, six resx entries) and
added about 60 (two tests, two localized labels in six cultures, the target shape doc).
