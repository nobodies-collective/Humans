...
| `Team/Details.cshtml` | 385 | `authorize-policy="@PolicyNames.BoardOrAdmin"` | "Roster" link (matches `TeamAdminController.Roster`'s action-level policy) |
...
| `src/Humans.Web/Controllers/TeamController.cs` | 160 | `AuthorizeAsync(User, teamInfo, TeamOperationRequirement.ManageEarlyEntry)` (drives `CanManageEarlyEntry` view-model flag on team details) |
| `src/Humans.Web/Controllers/TeamController.cs` | 679 | `AuthorizeAsync(User, PolicyNames.AdminOnly)` (EditTeam POST — `IsSensitive` leave-unchanged guard for non-Admin editors) |
...
| `src/Humans.Web/Controllers/StoreController.cs` | 54, 76, 79, 80, 81, 103, 120, 138, 172, 190, 219, 249, 276, 301 | `AuthorizeAsync(User, order/resource, StoreOrderOperationRequirement.X)` (and `StoreOrderCreateContext` / `StoreOrderLineContext` for Create at 103 and line-deadline-aware AddLine/RemoveLine at 249/276) |
...
| `src/Humans.Web/Controllers/ProfileController.cs` | 713, 746, 791, 833, 870, 907, 944, 964, 1008, 1087, 1103, 1129, 1162, 1188, 1214, 1390, 1416, 1460 | `AuthorizeAsync(User, userId, UserEmailOperations.Edit)` (18 email-edit endpoints) |
| `src/Humans.Web/Controllers/ProfileController.cs` | 1872 | `AuthorizeAsync(User, PolicyNames.TicketAdminBoardOrAdmin)` (onsite-chip visibility gate) |
| `src/Humans.Web/Controllers/ProfileController.cs` | 1968 | `AuthorizeAsync(User, PolicyNames.PrivilegedSignupApprover)` (inside `BuildSentMessagesContextAsync` private helper — gates whether a non-own-profile viewer sees the "sent messages" panel on the profile page; admits coordinators or `PrivilegedSignupApprover` role) |
