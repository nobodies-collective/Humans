...
**The layers are roles now, not projects.** Three kinds of project exist — `src/Humans.Base` (the base), `src/Sections/Humans.<Section>[.Contracts]` (42 section projects, 22 with a paired `.Contracts` leaf), and `src/Humans.Web` (the Shell). Dependency direction: a section may reference the base and other sections' `.Contracts` leaves; nothing may reference the Shell.
...
Repository **implementations** (the classes that talk to `DbContext`) live in the owning section project's `Data/`. Only the platform context lives in `Humans.Web/Data/`.
...
**There is no single context.** Each section has its own `internal sealed <Section>DbContext` mapping only that section's tables, with its own `__EFMigrationsHistory_<Section>` table and its own migrations folder — `src/Sections/Humans.<Section>/Data/Migrations/`, and `src/Humans.Web/Migrations/System/` for the platform context. There are **29 section contexts plus `SystemDbContext`**; the merged Users+Profiles section (`UsersDbContext`) carries the Identity base. Consequences:
...
The table-owning sections are listed below. The other section projects own no tables and so have no row: Cantina, Debug, Development, EarlyEntry, Gdpr, Guide, Monitor, Onboarding, Scanner, Search, Stripe, TicketTailor, Tour — several appear anyway, with an explicit *(no owned tables)* note, because a reader looking for them expects to find them here.
...
| **Users/Identity** | `UserService`, `AccountProvisioningService`, `UnsubscribeService`, `AccountMergeService`, `DuplicateAccountService`, `ExternalLoginService`, `AccountDeletionService`, `HumanLifecycleService` (`Humans.Users`, published via `Humans.Users.Contracts`) | `users`, `user_claims`, `user_logins`, `user_tokens`, `roles` (legacy), `user_roles` (legacy), `role_claims` (legacy), `event_participations`, `account_merge_requests` — the ASP.NET Identity tables are renamed to the PostgreSQL snake_case convention in `UsersDbContext.OnModelCreating`, which also carries the Identity base since peel 15 |
| **Teams** | `TeamService`, `TeamPageService` (composer — owns no tables) — `Humans.Teams`, published via `Humans.Teams.Contracts` | `teams`, `team_members`, `team_join_requests`, `team_join_request_state_history`, `team_role_definitions`, `team_role_assignments`, `team_early_entry_grants` |
| **Auth** | `RoleAssignmentService` (`Humans.Auth`, published via `Humans.Auth.Contracts`), `MagicLinkService` (owns no tables; `Humans.Auth.Services`, published via `Humans.Auth.Contracts.IMagicLinkService`) | `role_assignments` |
| **Backdoor** | `BackdoorApiKeyService` (`Humans.Backdoor`; section-internal — nothing outside Backdoor consumes it, so there is no contracts leaf) | `backdoor_api_keys` |
...
| **Onboarding** | `OnboardingService` (intake funnel). `HumanLifecycleService` (suspend/unsuspend state-machine) lives in **Users** — Peter, 2026-08-14: membership machinery is Users, never Governance — and is published via `Humans.Users.Contracts` | *(no owned tables — orchestrator over Profiles, Consent, Teams, Governance)* |
...
| **Containers** | `IContainerService` (`Humans.Containers`; implemented by the section-internal `Service`) | `containers`, `container_images`, `container_placements` |
...
| **Finance** | `IHoldedFinanceService` (`Humans.Finance`; implemented by the section-internal `Service`) | `holded_expense_docs`, `holded_category_map`, `holded_creditor_contacts`, `holded_doc_sync_state` |
| **Holded** | `IHoldedService` (`Humans.Holded`, published via `Humans.Holded.Contracts`; implemented by the section-internal `Service`) — the Holded API v2 ledger mirror, its reconciliation sweep, and the `/Holded` admin screen | `holded_ledger_lines`, `holded_accounts`, `holded_sync_states`, `holded_api_calls` |
| **Tickets** | `TicketQueryService`, `TicketSyncService`, `TicketTransferService` (`Humans.Tickets`, published via `Humans.Tickets.Contracts`; the vendor adapter is the separate `Humans.TicketTailor` project behind `ITicketVendorService`; the Tickets→Budget bridge `TicketingBudgetService` is **Budget**-owned — `src/Sections/Humans.Budget/Services/`) | `ticket_orders`, `ticket_attendees`, `ticket_sync_state`, `ticket_transfer_requests` |
| **Store** | `Service` (`Humans.Store`, internal — no public service interface; Store is self-contained and takes no cross-section calls into it) | `store_products`, `store_orders`, `store_order_lines`, `store_payments`, `store_invoices`, `store_treasury_sync_state` |
| **Scanner** | none (`Humans.Scanner`; no business logic and no `Services/` folder — its controller reads via `ITicketServiceRead`) | none |
| **Gate** | `GateService` (`Humans.Gate`) | `gate_scan_events`, `gate_settings`, `gate_staff_pins` |
...
| **Audit Log** | `AuditLogService` (`Humans.AuditLog`, published via `Humans.AuditLog.Contracts`) | `audit_log` |
| **Agent** | `AgentService`, `AgentSettingsService`, `AgentPromptAssembler`, `AgentToolDispatcher`, `AgentUserSnapshotProvider`, `AgentAbuseDetector`, `AnthropicClient`, `AgentConversationRetentionJob` (`Humans.Agent`) | `agent_conversations`, `agent_messages`, `agent_settings` |
...
**The Web→section decoupling epic (nobodies-collective/Humans#1073) is the general form of this shape.** Where §8a/§8b's three fanouts assemble a per-user *document*, the seam interfaces in `Humans.Base.Interfaces` let a section register itself into a piece of Shell *chrome* — nav, dashboard tiles, jobs, policies, health checks — without Shell naming the section. Each derives from the marker `ISectionContribution`, discovered by `SectionDiscoveryExtensions.DiscoverImplementations<T>()` — the same dependency-graph walk `ISection` uses — and registered as a singleton against every seam interface a contribution implements. A contribution is a **separate `internal sealed` stateless class at the section project's root, one file per seam, named `Section<Seam>`** (`internal sealed class SectionNav : ISectionNav`, `internal sealed class SectionJobs : ISectionJobs`), with a parameterless constructor — never a method added to `Section.cs`, which stays the DI-registration entry point only. The `Section` prefix isn't decoration: a bare `Jobs` class collides with the section's own `Jobs` namespace (`class Jobs` in `Humans.Consent` is CS0101 against `Humans.Consent.Jobs`, and a `Jobs()` method inside it is CS0542 on top), and the prefix removes that whole class of collision. **Internal, not public:** `DiscoverImplementations<T>()` walks `GetTypes()` rather than `GetExportedTypes()` precisely so a contribution need not be public — Shell reaches it by reflection and no other section ever names it, so it was never section-public surface (`Contracts/`) to begin with; making it public would also add a new public type per section per seam for nothing. Deriving a new seam interface from the marker costs Shell no edit.
...
| `NotificationService` | In-app notifications (`Humans.Notifications`) | `notifications`, `notification_recipients` |
...
The `TrackedCache` decorator stack below is the target caching architecture for any section that warrants one. `CachingUserService` (`src/Sections/Humans.Users/Data/CachingUserService.cs`, owning the `UserInfo` read model) is the reference implementation; the pattern was first proven on the Profile migration (PR #235, 2026-04-20) and every section that caches a canonical domain projection follows it. (Short-TTL request-acceleration caches — the `ApplicationServicesTakeNoMemoryCacheRule` allowlist — are a different, sanctioned shape; see §15i.)
...
The application service (`UserService`, `ProfileService`, `ContactFieldService`, etc.) lives in its section's `Humans.<Section>.Services` namespace and is `internal sealed` per HUM0034. (A handful of Users files still carry the legacy `Humans.Application.Services.*` namespace; namespace normalization retires those.) It:
...
Every section lives in its own project, with services in `Humans.<Section>.Services` (`internal` per HUM0034) and, for table-owning sections, repositories in `Humans.<Section>.Data` behind the section's own `<Section>DbContext`. (Tableless sections — e.g. Onboarding, EarlyEntry, Gdpr, Guide — correctly ship neither.)
